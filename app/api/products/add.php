...
function generate_sku($productCode, $variantName, $variantNumber)
{
    ...
    $cleanVariant = $variantName ? strtoupper(preg_replace('/[^A-Z0-9]/', '', substr($variantName, 0, 3))) : 'VAR';
    if (strlen($cleanVariant) < 2)
        $cleanVariant = 'VAR';

    return $productCode . '-' . $cleanVariant . '-' . str_pad($variantNumber, 2, '0', STR_PAD_LEFT);
}
...
try {
    ...
    $hasVariants = isset($_POST['variants']) && is_array($_POST['variants']) && count($_POST['variants']) > 0;
    ...
    if ($hasVariants) {
        ...
        // Add default variant if no variants specified
        ...
        $variantData = [
            ...
            'reorder_level' => !empty($_POST['reorder_level']) ? (int) $_POST['reorder_level'] : 10,
        ...
        // Add product image as variant image
        if ($mainImageUrl) {
    ...
    if (isset($_POST['stock_allocations']) && is_array($_POST['stock_allocations'])) {
        foreach ($_POST['stock_allocations'] as $allocationKey => $allocation) {
            ...
            $variantIndex = $allocation['variant_id'];
            $variantId = null;
            ...
            $locationId = !empty($allocation['location_id']) ? $allocation['location_id'] : null;

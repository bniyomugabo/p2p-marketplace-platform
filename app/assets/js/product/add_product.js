...
$(document).ready(function() {
    ...
    function refreshStockAllocations() {
        ...
        if ($variants.length === 0) {
            $container.html('<p class="text-muted text-center">Add variants first to allocate stock</p>');
            return;
        }
    ...
    function updateReviewPanel() {
        ...
        if ($variants.length === 0) {
            variantsHtml.push('<p class="text-muted">No variants added (simple product)</p>');
        } else {
            $variants.each(function(index) {
    ...
    function displayProductDetails(product) {
        ...
        if (product.variants && product.variants.length > 0) {
            ...
            variantsHtml = '<p class="text-muted">No variants</p>';
        }
            ...
            <div class="row">
                <div class="col-md-6">
                    ...
                    <table class="table table-sm">
                        <tr><th>Code:</th><td>${escapeHtml(product.product_code)}</td></tr>
                        <tr><th>Name:</th><td>${escapeHtml(product.product_name)}</td></tr>
                        <tr><th>Category:</th><td>${escapeHtml(product.category_name || '-')}</td></tr>
                        <tr><th>Brand:</th><td>${escapeHtml(product.brand || '-')}</td></tr>
                        <tr><th>Unit of Measure:</th><td>${escapeHtml(product.unit_of_measure)}</td></tr>
                        <tr><th>Description:</th><td>${escapeHtml(product.description || '-')}</td></tr>
                    </table>
                </div>
                <div class="col-md-6">
                    <h6>Stock Information</h6>
                    <table class="table table-sm">
                        <tr><th>Total Stock:</th><td>${formatNumber(product.total_stock || 0)}</td></tr>
                        <tr><th>Variants:</th><td>${product.variant_count || 0}</td></tr>

            ...
            <div class="card shadow h-100">
                <div class="card-header bg-white py-3 d-flex justify-content-between align-items-center">
                    ...
                    <div class="btn-group" role="group">
                        <button type="button" class="btn btn-sm btn-outline-primary active" id="btn-week">Week</button>
                        <button type="button" class="btn btn-sm btn-outline-primary" id="btn-month">Month</button>
                        <button type="button" class="btn btn-sm btn-outline-primary" id="btn-year">Year</button>
                    </div>
                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('[onclick="changeChartPeriod(\'week\')"], [onclick="changeChartPeriod(\'month\')"], [onclick="changeChartPeriod(\'year\')"]').forEach(btn => {
        btn.removeAttribute('onclick');
    });
    
    document.getElementById('btn-week')?.addEventListener('click', () => changeChartPeriod('week'));
    document.getElementById('btn-month')?.addEventListener('click', () => changeChartPeriod('month'));
    document.getElementById('btn-year')?.addEventListener('click', () => changeChartPeriod('year'));
});
...
function initializeCharts() {
    ...
    if (weeklyCanvas) {
        ...
        salesChart = new Chart(weeklyCanvas, {
            type: 'line',
            data: {
            ...
            options: {
                ...
                scales: {
                    y: {
                        ...
                        title: {
                            display: true,
                            text: 'Sales Amount (<?php echo $companyCurrency; ?>)',
                            font: { size: 12 }
                        },
                        ticks: {
                        ...
                        callbacks: {
                            label: function(context) {
                                ...
                                if (context.dataset.label.includes('Sales')) {
                                    return `${label}: <?php echo $companyCurrency; ?> ${value.toLocaleString()}`;
                                }
                                return `${label}: ${value}`;
                            }
                        }
    ...
    if (statusCanvas && <?php echo count($statusValues); ?> > 0) {
                    ...
                    tooltip: {
                        callbacks: {
                            label: function(context) {
                                ...
                                const total = <?php echo $totalStatusValue; ?>;
...
function changeChartPeriod(period) {
    ...
    fetch(`?page=api/dashboard/chart-data&period=${period}&company_id=<?php echo $companyId; ?>`)
        .then(response => response.json())
        .then(data => {
            if (data.success && salesChart) {
                ...
                salesChart.data.datasets[1].data = data.invoices;
                salesChart.update();
...
// Add IDs to the buttons in the HTML
document.addEventListener('DOMContentLoaded', function() {
    // Ensure buttons have IDs for event handlers
    const btnGroup = document.querySelector('.btn-group');
    if (btnGroup) {
        const buttons = btnGroup.querySelectorAll('.btn-outline-primary');
        if (buttons.length >= 3) {
            buttons[0].id = 'btn-week';
            buttons[1].id = 'btn-month';
            buttons[2].id = 'btn-year';
        }
    }
});
</script>
